...
if nargin < 3; BNZ = 1;                     end % Default binarization on
if nargin < 4; SCL = ceil(size(img,1) / 2); end % Envelope size to half the width of the image
...
bndsOut = [-getDim(ulng, 2) , getDim(ulng, 1)] + crds; % Right side
bndsInn = [getDim(ulng, 2) , -getDim(ulng, 1)] + crds; % Left side

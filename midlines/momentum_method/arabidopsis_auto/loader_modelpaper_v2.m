...
parfor e = 1:numel(FileList)
    tic
    out(e) = loadData(FileList{e},20,200,200);
    toc
end
%% ingestigate the ME
for e = 1:numel(out)
...
comp = zeros(2,301,200,numel(out));
for e = 1:numel(out)
    if isempty(out(e).ME) & (size(out(e).K,2) == 301) & (size(out(e).K,3) == 1)
        MK(:,:,e) = out(e).K;
        angle(:,e) = out(e).angle;
...
figure(f_ta);
legend(LEG);
%% INSERT NEW MODEL HERE - ATTEMPT ONE
nd = size(comp);
comp_sm = reshape(comp,[prod(nd(1:end-1)) nd(end)]);
[smS smC smU smE smL smERR smLAM] = PCA_FIT_FULL(comp_sm',7);
%% continue here
kidx = kmeans(mean(growthRate,1),8);
UQ = unique(kidx);
for u = 1:numel(UQ)
    fidx = find(kidx==UQ(u));
    uG(u) = mean(mean(growthRate(:,fidx)));
end
[uG sidx] = sort(uG);
for u = 1:numel(UQ)
    kidx_tmp(kidx == UQ(u)) = sidx(u);
end
kidx = kidx_tmp;
UQ = unique(kidx);
touse = 3;
for u = 1:numel(UQ)
    fidx = find(UQ(u) == kidx);
    sub = smC(fidx,:);
    [subS subC subU(u,:) subE(:,:,u) subL subERR subLAM] = PCA_FIT_FULL(sub,2);
    plot3(sub(:,1),sub(:,2),sub(:,3),'.')    
    hold all
end

%% X1,X2 as a function of latent
X1 = squeeze(comp(1,:,:,:));
X2 = squeeze(comp(2,:,:,:));
...
[XS1 XC1 XU1 XE1 XL1 XERR1 XLAM1] = PCA_FIT_FULL([X1'],3);
[XS2 XC2 XU2 XE2 XL2 XERR2 XLAM2] = PCA_FIT_FULL([X2'],3);
...
%% HERE AND BELOW IS THE BEST SO FAR
%% AND AGAIN WITH the TA and growth rate direct summed
X1 = squeeze(comp(1,:,:,:));
X2 = squeeze(comp(2,:,:,:));
[gS gC gU gE gL gERR gLAM] = PCA_FIT_FULL(growthRate',4);
...
X3 = growthRate;
X4 = angle;
[XS XC XU XE XL XERR XLAM] = PCA_FIT_FULL([X1' X2' X3' X4'],3);

X1  = reshape(X1,sz1);
X2  = reshape(X2,sz2);

X1S = XS(:,1:sz1(1)*sz1(2))';
X2S = XS(:,(sz1(1)*sz1(2)+1):(sz1(1)*sz1(2)+sz1(1)*sz1(2)))';
X3S = XS(:,(sz1(1)*sz1(2)+sz1(1)*sz1(2)+1):(sz1(1)*sz1(2)+sz1(1)*sz1(2)+1)+size(growthRate,1));

X1S  = reshape(X1S,sz1);
X2S  = reshape(X2S,sz2);

compS = cat(1,shiftdim(X1S,-1),shiftdim(X2S,-1));
%% AND AGAIN WITHOUT the TA and growth rate direct summed
X1 = squeeze(comp(1,:,:,:));
X2 = squeeze(comp(2,:,:,:));
...
compS = cat(1,shiftdim(X1S,-1),shiftdim(X2S,-1));
%% kernel pca for non-linear try
sigma = 3000;
[K_n,V,tp] = KPCA(XC(:,1:2),10,sigma);
[n1 n2] = ndgrid(linspace(-1*10^4,1*10^4,300),linspace(-1*10^4,1*10^4,300));
tpt = [n1(:) n2(:)];
[ktp] = projectTPToVforK(K_n,XC,V,sigma,tpt);
%%
G = reshape(ktp(:,2),[300 300]);
figure;
mesh(linspace(-1*10^4,1*10^4,300),linspace(-1*10^4,1*10^4,300),G);
hold on
scatter(XC(:,1),XC(:,2),'k.')
figure;contour(G,100)
%% view and again
figure
...
%% look at PCA for tip angle vs unwrapped
for e = 1:size(CS,2)
    figure;
    plot(CS(:,e),aC(:,1),'.');
end
%% FRAME MODEL FOR AND AGAIN
%% frame modeling
...
%% try parameter maps
[gS gC gU gE gL gERR gLAM] = PCA_FIT_FULL(growthRate',2);
[aS aC aU aE aL aERR aLAM] = PCA_FIT_FULL(angle',4);

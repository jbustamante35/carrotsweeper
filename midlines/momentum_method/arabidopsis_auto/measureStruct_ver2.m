function [S] = measureStruct_ver2(S,NP,NPK,SNIP,sz)
    try
        [angle length K snip frameOut snipC] = measurePhenoTypesOverStack(S.frameData,NP,NPK,1,SNIP);
        S.angle = angle;
        S.length = length;
        S.K = -permute(K,[2 3 1]);

...
#scanTime=$(/usr/bin/time -f %e 2>&1 scanimage --mode color -x 55 -y 55 -d "epson2:libusb:$3:$4" --resolution $2 --format tiff > $1)
scanTime=$(/usr/bin/time -f %e 2>&1 scanimage --mode color -x 133 -y 133 -d "epson2:libusb:$3:$4" --resolution $2 --format tiff > $1)

dPath = '/home/nate/Downloads/AprilData/';
oPath = '/home/nate/Downloads/forApril3/';
mkdir(oPath);
cdir = dir(dPath);
cdir(1:2) = [];
close all
...
for l = 1:numel(cdir)
    ...
    R = regionprops(rM,'Image','BoundingBox','Centroid');
    ...
    for e = 1:numel(R)
        try
            ...
            drawnow
            imwrite([vec Model],[oPath cdir(l).name '_'  num2str(e) '_straight.tif']);
            pause(.001)
            %waitforbuttonpress
            ...
            saveas(h2,[oPath cdir(l).name  '_whole.tif']);
            %waitforbuttonpress
        catch ME
            getReport(ME)

function [] = main_swell_HT(inFilePath,oPath)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% scan for new images
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    FileList = {};
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for e = 1:numel(SET)
        [swell{e} area{e} para{e}] = generteOutFileBase(SET{e}{1},oPath,1);
    end
    %{
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%% analyze the data
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    mainBOX = [424 195 9160 10425]; % fixed crop box due to template
    parfor e = 1:numel(SET)
        % measure the data
        [area{e} dB{e}] = measureGerminate(SET{e},[700 700],mainBOX,numel(SET{e}),0);
...
%{
    ...
    main_swell_HT(inFilePath,oPath);
%}
...
for genoType = 1:size(bS,1)
    ...
    drawnow
    %pause(.2)
    COR = [COR ; [x0{genoType} x0{genoType}(:,1).*x0{genoType}(:,2).^-1 tmpArea(:,1)]];
end
figure(h1);
legend(LEG);
figure(h2);

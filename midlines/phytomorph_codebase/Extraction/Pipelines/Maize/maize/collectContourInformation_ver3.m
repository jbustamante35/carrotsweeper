function [curve] = collectContourInformation_ver3(I,OP)
    ...
    % generate containment map for contours and remove 'non-root' curves
    CM = generateContainmentMap(curve);
    rmidx = ~all(CM==1,2);
    containNumber = sum(CM,2);
    [cn, kpidx] = max(containNumber);
    curve = curve(kpidx);
    %curve(rmidx) = [];
    
    % normlize via arclength
    ...
    for e = 1:numel(curve)        
        ...
        curve(e).segs = generateCurveSegments(curve(e),OP.segmentLength);
        % display curves
        if OP.displayCurve
            close all
            imshow(fI,[])
            ...
            for e = 1:numel(curve)
                ...
                %quiver(curve(e).data(1,:)',curve(e).data(2,:)',squeeze(tmpCurve(e).E(1,2,:)),squeeze(tmpCurve(e).E(2,2,:)));
                drawnow
            end
        end
    end
...
function [CM] = generateContainmentMap(tmpC)
    ...
    for i = 1:numel(tmpC)        
        ...
        CM(i,:) = inpoly(PTL,tmpC(i).data(:,1:ds:end)');        
    end
end
%{
    fileName = SET{1}{1};

function [sIDX,LABELS] = getMap_ver1(tMASK)
        try
        ...
        [~,sidx0] = sort([R(Sidx(1)).Centroid(1);R(Sidx(2)).Centroid(1);R(Sidx(3)).Centroid(1)]);
        [~,sidx1] = min([R(Sidx(sidx0(1))).Centroid(2);R(Sidx(sidx0(2))).Centroid(2)]);

        Sidx = Sidx(sidx0(sidx1));
        CM = [];

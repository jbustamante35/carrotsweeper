...
parfor e = 1:N
    ...
    try
        ...
        I = imread(FileList{e});
        % get masks
        MASK{e} = generateCropBoxes(I);
        % crop 
        [sI{e},sM{e}] = verticalCrop_ver0(I,full(MASK{e}),newY);
        % re-size
        W(:,:,:,e) = imresize(I,.25);
...
X2 = zeros([size(sI{1},2) size(sI{1},1) 3 3*200]);
cnt = 1;
for e = 1:200
    for r = 1:3
        Y2 = [Y2 any(sM{e}(:,:,r),2)'];
        X2(:,:,:,cnt) = permute(sI{e}(:,:,:,r),[2 1 3]);
        cnt = cnt + 1;
    end
    e
end
...
clear sI
%%
[funcObject1] = getNetwork(W,5,10,Y);
...
[funcObject2] = getNetwork(X2,5,10,Y2);
...
[funcObject3] = getNetwork(X3,2,10,Y3);

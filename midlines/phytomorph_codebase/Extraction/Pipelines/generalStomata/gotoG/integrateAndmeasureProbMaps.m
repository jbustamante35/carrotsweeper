function [grade,BO,wSUM] = integrateAndmeasureProbMaps(source,target,para,oI)
    grade = NaN;
    try
...
%}
        ...
        for o = 1:size(source,4)
            ...
            for slice = 1:size(tmpS,3)
                ...
                tmpS(:,:,slice) = imfilter(tmpS(:,:,slice),fspecial('gaussian',[21 21],max(round(para(11)),1)),'replicate');
            ...
            BO = imfill(BO,'holes');
            BO = bwareaopen(BO,max(round(para(12)),1));
            
            
            BO2 = wSUM > para(13);
            BO2 = imfill(BO2,'holes');
            BO2 = bwareaopen(BO2,max(round(para(12)),1));
            
            BO = logical(BO.*BO2);
            
            
            
            centerBO = imclearborder(BO);
            edgeBO = logical(BO - centerBO);
            
            
            centerBO = bwareaopen(centerBO,max(round(para(14)),1));
            edgeBO = bwareaopen(edgeBO,max(round(para(15)),1));
            
            BO = logical(centerBO + edgeBO);
            
            if ~isempty(target)
                
                R = regionprops(logical(target(:,:,o)));
                numT(o) = numel(R);
                ...
                [grade(o)] = -matthews_correlation(logical(TOTX),logical(TOTY));
                %[grade] = -positiveLikehoodRatio(logical(TOTX),logical(TOTY));
            end
        ...
        if ~isempty(target)
            grade(isnan(grade)) = -1;
            grade = (mean(grade) - corr(numS',numT'))/2;
        end

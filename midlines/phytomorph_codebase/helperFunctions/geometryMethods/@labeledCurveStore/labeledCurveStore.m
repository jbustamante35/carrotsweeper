classdef labeledCurveStore < curveStore
    properties
        labels;
    end
    ...
    methods
        ...
        function [obj] = labeledCurveStore()

        end
        
        function [] = insertParticles(obj,particles)
            insertParticles@curveStore(obj,particles);
            for e = 1:numel(particles)
                obj.labels = [obj.labels particles(e).labelVec];
            end
        end
        
        function [dist] = distanceToStore(obj,curveSegment,sidx)
            dist = [];
            if nargin == 2;sidx = 1:numel(obj.store);end
            for e = 1:numel(sidx);
                dist(e,:) = obj.store{sidx(e)}.distance(curveSegment);
            end
        end 
       
        function [uDist sDist] = selfDistance(obj,labelNumber)
            if ~isempty(obj.labels)
                sidx = find(obj.labels(labelNumber,:));
                for e = 1:numel(sidx)
                    tmpD = obj.distanceToStore(obj.store{sidx(e)},setdiff(sidx,sidx(e)));
                    D(e,:) = mean(tmpD,1);
                end
                if isempty(D)
                    uDist = 0;
        ...
        function [labels] = predictTipParticle(obj,particleEnsamble)
            labels = zeros(1,numel(particleEnsamble));
            if ~isempty(obj.labels)
                sidx = find(obj.labels(1,:));
                [uDist sDist] = obj.selfDistance(1);
                for p = 1:numel(particleEnsamble)
                    dist = obj.distanceToStore(particleEnsamble(p),sidx);
                    dist = mean(dist,1) - uDist;
                    dist = dist.*sDist.^-1;
                    D(p) = sum(abs(dist([2 3])),2);
                end
                [J sidx] = min(D);
                labels(sidx) = 1;
            end
        ...
        function [D] = distanceToLabelParticles(obj,particleEnsamble,labelNumber)
            if ~isempty(obj.labels)
                sidx = find(obj.labels(labelNumber,:));
                for p = 1:numel(particleEnsamble)
                    dist = obj.distanceToStore(particleEnsamble(p),sidx);
        ...
        function [labels] = predictJunctionUpper(obj,particleEnsamble)
            labels = zeros(1,numel(particleEnsamble));
            if ~isempty(obj.labels)
                sidx = find(obj.labels(2,:));
                [uDist sDist] = obj.selfDistance(2);
                for p = 1:numel(particleEnsamble)
                    dist = obj.distanceToStore(particleEnsamble(p),sidx);
                    dist = mean(dist,1) - uDist;
                    dist = dist.*sDist.^-1;
                    D(p) = sum(abs(dist([3])),2);
                end
                [J sidx] = min(D);
                labels(sidx) = 1;
            end
        ...
        function [labels] = predictJunctionLower(obj,particleEnsamble)
            labels = zeros(1,numel(particleEnsamble));
            if ~isempty(obj.labels)
                sidx = find(obj.labels(3,:));
                [uDist sDist] = obj.selfDistance(3);
                for p = 1:numel(particleEnsamble)
                    dist = obj.distanceToStore(particleEnsamble(p),sidx);
                    jD(p,:) = mean(dist,1);
                    dist = mean(dist,1) - uDist;

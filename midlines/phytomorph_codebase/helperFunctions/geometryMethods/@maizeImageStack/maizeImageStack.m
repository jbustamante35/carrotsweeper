classdef maizeImageStack 
    ...
    methods
        ...
        function [obj] = maizeImageStack(imageStack)
            for e =1:numel(imageStack)
                obj.imageStack{e} = maizeImage(imageStack{e});
            end
        end
        ...
        function [count] = getKernelCount(obj)
            if isempty(obj.count)
                % extract the kernel centers
                kernelCenters = obj.imageStack{1}.extractKernelCenterPoints();
                obj.count = size(kernelCenters,2);
            end
            count = obj.count;
        end
        
        function [kernelFileNames] = getKernelFileNames(obj,savePath)
            name = obj.getName;
            for k = 1:obj.getKernelCount()
                kernelFileNames{k} = [savePath name '--' num2str(k) '.mat'];
            end
        end
        ...
        function [curveSequence] = extractContourSequences(obj)
            ...
            curveSet = obj.imageStack{1}.extractClosedContours(@(x)maizeContour(x),10,1,[100 inf]);
            ...
            for e = 1:numel(repCurveSet)
                ...
                for f = 1:numel(obj.imageStack)-1
                    ...
                    tmpContours = I.extractClosedContours(@(x)maizeContour(x),30,3,[100 inf]);

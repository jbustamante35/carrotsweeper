...
%   $Revision: 1.1.6.21 $  $Date: 2011/05/17 02:25:06 $

[filename, fmt_s, extraArgs, msg] = parse_inputs(varargin{:});

% added via nate
verbose = 0;
if ~isa(filename,'imageFile')
    [junkPath junkName junkExt] = fileparts(filename);
else
    [junkPath junkName junkExt] = fileparts(filename.fileName);
end

if (~isempty(msg))
    error(message('MATLAB:imagesci:imread:inputParsing', msg));
end




if ~isa(filename,'imageFile') && isempty(strfind(junkExt,'.nms'))
    ...
    if (strfind(filename, '://'))
        ...
        try
            ...
            if strcmp(lower(tE),'.nef')
                tName = [tempname '.nef'];
            else
                tName = tempname;
            end
            filename = urlwrite(filename, tName);
        catch ME%#ok<*CTCH>
            getReport(ME)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% added for routing - for pure filename of irods
toDelete = 0;
if isa(filename,'imageFile') && ~isempty(strfind(junkExt,'.nms'))
    if isIRODS(filename.fileName)
        url = true;
    else
        url = false;
    end
...
elseif isIRODS(filename)
    toDelete = isIRODS(filename) & isdeployed();
    if strcmp(filename(end),'#')
        
        fidx = strfind(filename,'#');
        iticket = filename(fidx(end-1)+1:fidx(end)-1);
        
        
        filename = filename(1:fidx(end-1)-1);
        fileList = {filename};
        tmpPath = [tempname filesep];
        mkdir(tmpPath);
    
        tm = clock;
        fileList = xfer_get(fileList,tmpPath,0,0,{iticket});
        fprintf(['File xfer time:' num2str(etime(clock,tm)) '\n']);
        fileList = fileList{1};
        filename = fileList;
        url = true; 
...
if verbose;fprintf(['imread@phytoLib@myAuto\n']);end;
classType = class(filename);
fidx = strfind(classType,'.');
if ~isempty(fidx)
    classType = classType(fidx(end)+1:end);
end

if strcmp(classType,'Buri_file')
    import phytoG.locked.Bpersist.Bfs.abstractions.*;  
    filename = char(filename.getFullFileName());
    if verbose;fprintf([classType '@phytoGraph@' filename  '\n']);end
end


%[tmpP tmpN tmpE] = fileparts(filename);

if ~isa(filename,'imageFile')
    [tmpP tmpN tmpE] = fileparts(filename);
else
    [tmpP tmpN tmpE] = fileparts(filename.fileName);
end
if strcmp(lower(tmpE),'.nef')
    fprintf(['Converting NEF -> TIF \n']);
    CMD = ['dcraw -a -T "' filename '"'];
    ...
    filename = strrep(filename,tmpE,'.tiff');
end
...
if (isempty(fmt_s))
    ...
    if (isempty(format)) & ~strcmp(tmpE,'.nms')
...
%%%
function [filename, fmt_s, extraArgs, msg] = parse_inputs(varargin)
...
case 1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % get class type - java
    try classType =  char(filename.getClass().getSimpleName());catch ME;classType='NA';end;
    % get class type - matlab
    if strcmp(classType,'NA');classType = class(filename);end
    if ~ischar(filename) & ~strcmp(classType,'Buri_file') & ~strcmp(classType,'imageFile')

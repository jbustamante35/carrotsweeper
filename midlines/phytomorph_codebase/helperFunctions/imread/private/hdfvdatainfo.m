...
%   $Revision: 1.1.6.11 $  $Date: 2011/05/17 02:27:38 $
...
elseif ischar(dataset)
    ...
    if ref == 0
        warning(message('MATLAB:imagesci:hdfvdatainfo:find', dataset));
        return;
    end
end
...
if vdID == -1
    warning(message('MATLAB:imagesci:hdfvdatainfo:attach'));
    return;
end
...
if (status == -1)
    ...
    warning (message('MATLAB:imagesci:hdfvdatainfo:inquireFailure', ref, name, ext));
    records = '';
    fields = '';
else
    %Parse field names

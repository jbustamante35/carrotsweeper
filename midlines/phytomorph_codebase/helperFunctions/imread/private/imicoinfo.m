...
%   $Revision: 1.1.6.7 $  $Date: 2011/05/17 02:27:47 $


[fid, msg] = fopen(filename, 'r', 'ieee-le');  % ICO files are little-endian
if (fid == -1)
   error(message('MATLAB:imagesci:imicoinfo:fileOpen', filename, msg)); 
end
...
if (isempty(sig))
    fclose(fid);
    error(message('MATLAB:imagesci:imicoinfo:fileEmpty', filename));
end
    
if (~isequal(sig, [0; 1]))
    fclose(fid);
    error(message('MATLAB:imagesci:imicoinfo:notICOFile', filename));
end
...
for p = 1:imcount
    ...
    if (count ~= info(p).NumColormapEntries*4)
        fclose(fid);
        error(message('MATLAB:imagesci:imicoinfo:truncatedColormap'));
    end
    ...
    if ((info(p).Width < 0) || (info(p).Height < 0))
        fclose(fid);
        error(message('MATLAB:imagesci:imicoinfo:badImageDimensions'));
    end
end

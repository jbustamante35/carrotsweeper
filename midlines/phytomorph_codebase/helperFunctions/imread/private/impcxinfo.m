...
%   $Revision: 1.1.6.6 $  $Date: 2011/05/17 02:27:50 $

[fid, msg] = fopen(filename, 'r', 'ieee-le');
if (fid == -1)
    error(message('MATLAB:imagesci:impcxinfo:fileOpen', filename, msg));
end
...
if (fseek(fid, 128, 'bof') ~= 0)
    fclose(fid);
    error(message('MATLAB:imagesci:impcxinfo:headerTooSmall', filename));
end
...
if (info.FormatSignature(1) ~= 10)
    fclose(fid);
    error(message('MATLAB:imagesci:impcxinfo:notPCXFile', filename));
end

info.FormatVersion = info.FormatSignature(2);
if (~ismember(info.FormatVersion, [0 2 3 4 5]))
    fclose(fid);
    error(message('MATLAB:imagesci:impcxinfo:unrecognizedPCXFormat'));
end

...
%   $Revision: 1.1.6.6 $  $Date: 2011/05/17 02:27:55 $
...
if (fid == -1)
    error(message('MATLAB:imagesci:imxwdinfo:fileOpen', filename));
end
...
if (length(info.FormatSignature) < 2)
    fclose(fid);
    error(message('MATLAB:imagesci:imxwdinfo:truncatedHeaderSignature'));
end
...
if (info.FormatVersion ~= 7)
    ...
    if (fid == -1)
        error(message('MATLAB:imagesci:imxwdinfo:littleEndianfileOpen', filename));
    end
    info.FormatSignature = fread(fid, 2, 'uint32');
    info.HeaderSize = info.FormatSignature(1);
    info.FormatVersion = info.FormatSignature(2);
...
if (isempty(format))
   fclose(fid);
   error(message('MATLAB:imagesci:imxwdinfo:truncatedHeaderFormat'));
end
...
if (isempty(info.NumColormapEntries))
    fclose(fid);
    error(message('MATLAB:imagesci:imxwdinfo:truncatedHeaderColormapEntries'));
end

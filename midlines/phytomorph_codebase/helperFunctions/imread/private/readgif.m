...
%   $Revision: 1.1.6.12 $  $Date: 2011/05/17 02:28:00 $
...
if (nargin < 1)
    ...
    error(message('MATLAB:imagesci:readgif:tooFewInputs'));
...
elseif (nargin > 1)
    ...
    if (~isempty(msg))
        error(message('MATLAB:imagesci:readgif:inputArguments', msg))
    end
...
% Create a Java object to read the GIF data.
try
    g = com.mathworks.util.GIFObject(filename);
catch myException
    
    % When headless, we can't tell for sure whether that is the cuase of
    % the error.  So warn and then rethrow the Java error.
    env = java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment;
    if ((env.isHeadless) && (isunix))
        warning(message('MATLAB:imagesci:readgif:badHeadlessEnvironment'))
    end

    rethrow(myException);
end

% Read the image using requested options.
if (nargin == 1)
    ...
    if (~isempty(msg))
        error(message('MATLAB:imagesci:readgif:splitMultiframe', msg))
    end
...
while (separator ~= 59)  % 59 == EOS indicator.
    ...
    %   (2) any extensions which precede the next image block,
...
if (nargin > 1)
    ...
    for k = first_arg:2:length(varargin)
        ...
        idx = strncmp(param, paramStrings, numel(param));
        if (isempty(idx))

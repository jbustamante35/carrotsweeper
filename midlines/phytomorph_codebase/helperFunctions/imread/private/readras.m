...
%  $Revision: 1.1.6.8 $  $Date: 2011/05/17 02:28:08 $
   ...
   if fid < 0
      error(message('MATLAB:imagesci:readras:fileOpen', filename, msg));
   end
   ...
   if maplength
      ...
      if count < maplength
         fclose(fid);
         error(message('MATLAB:imagesci:readras:unexpectedEOFatColorMapRead', filename));
      end

      % Convert the color map to MATLAB type color map.
      switch info.MapType
         case RMT_NONE
            warning(message('MATLAB:imagesci:readras:badMapLength', filename));
         case RMT_EQUAL_RGB
         ...
         case RMT_RAW
            ...
            map = map(:,[1 1 1])/255;
         otherwise
            error(message('MATLAB:imagesci:readras:badMapType', filename));
      end
   ...
   switch info.BitDepth
      ...
      case 32
         ...
         numBytes = paddedByteWidth * height;
      otherwise
         error(message('MATLAB:imagesci:readras:badBitDepth', filename, info.BitDepth));
   end
   ...
   if info.Type ~= RT_BYTE_ENCODED && datalength ~= numBytes
      ...
      warning(message('MATLAB:imagesci:readras:badDataLength', filename));
   end
   ...
   if count < datalength
      warning(message('MATLAB:imagesci:readras:unexpectedEOFatImageRead', filename));
   ...
   if fsize > headersize + maplength + datalength
      warning(message('MATLAB:imagesci:readras:tooMuchData', filename));
   end
   ...
   switch info.Type
      ...
      case RT_FORMAT_TIFF
         error(message('MATLAB:imagesci:readras:tiffRasNotSupported', filename));
      case RT_FORMAT_IFF
         error(message('MATLAB:imagesci:readras:iffRasNotSupported', filename));
      case RT_EXPERIMENTAL
         error(message('MATLAB:imagesci:readras:experimentalRasNotSupported', filename));
      otherwise
         error(message('MATLAB:imagesci:readras:invalidRasType', filename));
   end

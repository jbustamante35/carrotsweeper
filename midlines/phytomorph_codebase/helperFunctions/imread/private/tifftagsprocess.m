...
%   $Revision: 1.1.8.10 $  $Date: 2011/05/17 02:28:11 $
...
case 0
    y = sprintf('%sno strobe return detection function, ', y);
case 1
    y = sprintf('%sreserved, ', y);
case 2
    y = sprintf('%sstrobe return light not detected, ', y);
case 3
    y = sprintf('%sstrobe return light detected, ', y);
end
...
case 0
    y = sprintf('%sunknown flash mode, ',y);
case 1
    y = sprintf('%scompulsory flash firing, ', y);
case 2
    y = sprintf('%scompulsory flash suppression, ', y);
case 3
    y = sprintf('%sauto flash mode, ', y);
end

%
% presence of flash function
if bitshift ( bitand(x,32), -4 )
    y = sprintf('%sno flash function, ', y);
else
    y = sprintf('%sflash function present, ', y);
end
...
case 0
    y = sprintf('%sno red-eye reduction mode or unknown.', y);
case 1
    y = sprintf('%sred-eye reduction mode supported.', y);
end

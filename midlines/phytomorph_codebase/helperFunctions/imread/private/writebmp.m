...
%   $Revision: 1.1.6.6 $  $Date: 2011/05/17 02:28:12 $

if (~isa(X,'logical') && ~isa(X,'uint8') && ~isa(X,'single') && ~isa(X,'double'))
    error(message('MATLAB:imagesci:writebmp:unsupportedImageClass'));
end
...
if ((numdims > 2) && ((numdims ~= 3) || (size(X,3) ~= 3)))
    error(message('MATLAB:imagesci:writebmp:badImageDimensions'));
end
...
if ((biBitCount==1) && (size(map,1) ~= 2))
    error(message('MATLAB:imagesci:writebmp:wrongNumberOfColormapEntriesFor1D'));
end

if ((numcomps == 1) && (size(map,1) > 256))
    error(message('MATLAB:imagesci:writebmp:wrongNumberOfColormapEntries'));
end

if ((numcomps == 1) && (size(map,2) ~= 3))
    error(message('MATLAB:imagesci:writebmp:invalidColormapSize'));
end

fid=fopen(filename,'W','l');
if (fid==-1)
    error(message('MATLAB:imagesci:writebmp:fileOpen', filename));
end;
...
if biBitCount==1
  ...
  if (fid==-1)
    error(message('MATLAB:imagesci:writebmp:fileReopen', filename));
  end;
  fseek(fid,0,'eof');
  fwrite(fid,X,'ubit1');
else

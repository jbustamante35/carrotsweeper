...
%   $Revision: 1.1.6.10 $  $Date: 2011/05/17 02:28:16 $
...
for k = 1:2:length(varargin)
    ...
    if (~ischar(param))
        error(message('MATLAB:imagesci:writejpg:badParameterName'));
    end
    
    idx = find(strncmp(param, paramStrings, numel(param)));
    if (isempty(idx))
        error(message('MATLAB:imagesci:writejpg:unrecognizedParameter', param));
    elseif (length(idx) > 1)
        error(message('MATLAB:imagesci:writejpg:ambiguousParameter', param));
    end
...
case 'quality'
    ...
    if (~isa(quality,'numeric'))
        error(message('MATLAB:imagesci:writejpg:nonNumericQualityValue'))
    end
    
    if ((quality < 0) || (quality > 100))
        error(message('MATLAB:imagesci:writejpg:badQualityValue'));
    end
...
case 'bitdepth'
    ...
    if (~isa(bits,'numeric'))
        error(message('MATLAB:imagesci:writejpg:nonNumericBitDepth'));
    end
...
case 'mode'
    ...
    if ((~ischar(mode)) || ...
        ...
        error(message('MATLAB:imagesci:writejpg:badMode'))

function header = parseHdrInfoStruct(path, name)
% parseHdrInfoStruct
%
% header = parseHdrInfoStruct(path,name)
% return header struct variable with the following fields:
%     .spatial
%     .frames
...
%     .datatype
%     .settings (Band Intensities - Acuity files)
    ...
    if numel(ind)
        ...
        if numel(ind) && ~exist('frames','var')
            ...
            if (ind == 1)
                ...
                ind = strfind(line, 'data type');
                if numel(ind)
                    datatype = str2double(line((strfind(line, '=')+1):size(line, 2)));
                else
...
header.wl = wavelengths;
header.spatial = spatial;
header.frames = frames;
header.spectral = spectral;
header.tint = tint;

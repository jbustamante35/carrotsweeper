function  currfig = zoom2cursor(I,P,G,B,mag)
% I     := image
% P     := point cluster
% G     := guesses for selection
% B     := boolean selection vector
% mag   := magnification
...
CL = {'yo','mo','co'};
for ng = 1:size(G,2)
    hold on;plot(P(find(G(:,ng)),2),P(find(G(:,ng)),1),CL{ng});
end
hold on;plot(P(:,2),P(:,1),'r*');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% user def mag
zoomparams.pct = mag; %Default value
...
%%% added by nate for storing the total feature points
setappdata(gcf, 'featurePoints',P);
%%% added by nate for storing the total feature points
setappdata(gcf,'selectedPoints',[]);
setappdata(gcf,'node',B);
setappdata(gcf,'imagePatches',G);
...
endbutton = uicontrol('style','pushbutton','string','X','units','normalized','position',[0 0 0.04 0.065],'foregroundcolor','y','backgroundcolor','k',...
    ...
    ['zoomparams = getappdata(gcf,''zoomparams'');set(gcf,''windowbuttonmotionfcn'',zoomparams.oldwbmf);',...
        ...
        'delete(zoomparams.dispbox1);delete(zoomparams.dispbox2);delete(gcbo);close(gcf);']);
...
case 'normal'
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% removed and replaced by nate
    posn = get(zoomparams.currax,'currentpoint');
    posn = posn(1,:);
    x0 = posn(1,1);
    y0 = posn(1,2);

    Po = getappdata(gcf, 'featurePoints');
    So = getappdata(gcf, 'selectedPoints');
    Go = getappdata(gcf, 'imagePatches');
    DEL = Po - repmat([y0 x0],[size(Po,1) 1]);
    DEL = sum(DEL.*DEL,2);
    [J idx] = min(DEL);
    So = [So;Po(idx,:)];
    
    plot(So(end,2),So(end,1),'go');
    
    setappdata(gcf, 'selectedPoints',So);
    T = getappdata(gcf, 'node');
    vec = T.getData;
    vec(idx) = 1;
    T.setData(vec,1);
    ...
    % removed zoom on left mouse click

...
%   $Revision: 1.1.6.14 $  $Date: 2011/05/17 02:27:37 $
...
try
    ...
    try
        ...
        error(message('MATLAB:imagesci:hdfswathread:fieldNotFound', fieldname, hinfo.Name));
    end
...
if length(fields)>1
  error(message('MATLAB:imagesci:hdfswathread:tooManyFields'));
end
...
if numFields >= numReqFields
  for i=1:numReqFields
    if ~isfield(hinfo,reqFields{i})
      error(message('MATLAB:imagesci:hdfswathread:missingRequiredField'));
    end
  end
else 
  error(message('MATLAB:imagesci:hdfswathread:invalidHinfoStruct'));
end
...
if ~isempty(params.Index) || ~isempty(params.Time) 
    ...
    if s > 1
        error(message('MATLAB:imagesci:hdfswathread:inconsistentParameters'));
    end
end
...
if ( numel(C) > 0 )
    error (message('MATLAB:imagesci:hdfswathread:incompatibleVerticalSubset', C{ 1 }, C{ 1 }));

...
%   $Revision: 1.1.6.9 $  $Date: 2011/05/17 02:27:39 $
...
if fileID == -1
  error(message('MATLAB:imagesci:hdfvdataread:interfaceOpen', hinfo.Name));
end
status = hdfv('start',fileID);
if status == -1
  hdfh('close',fileID);
  error(message('MATLAB:imagesci:hdfvdataread:interfaceStart', hinfo.Name));
end
...
if vdID == -1
  ...
  error(message('MATLAB:imagesci:hdfvdataread:interfaceAttach', hinfo.Name)) ;
end
...
if numFields >= numReqFields
  for i=1:numReqFields
    if ~isfield(hinfo,reqFields{i})
      error(message('MATLAB:imagesci:hdfvdataread:invalidHinfoFields'));
    end
  end
else 
  error(message('MATLAB:imagesci:hdfvdataread:tooFewHinfoFields'));
end
if ~isfield(hinfo.Fields,'Name')
  error(message('MATLAB:imagesci:hdfvdataread:missingHinfoName'));
end
...
for j = 1:numel(req_fields)
    ...
    if notThere
        error(message('MATLAB:imagesci:hdfvdataread:fieldNotPresent', req_fields{ j }));
    end
end
...
if (firstRecord > hinfo.NumRecords) 
    error(message('MATLAB:imagesci:hdfvdataread:badFirstRecord', hinfo.NumRecords));
elseif (numrecords > hinfo.NumRecords)
    error(message('MATLAB:imagesci:hdfvdataread:badRecordSpecification', numrecords, hinfo.NumRecords));
end

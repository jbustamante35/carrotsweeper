...
%   $Revision: 1.1.6.7 $  $Date: 2011/05/17 02:27:42 $
...
if (fid == -1)
    error(message('MATLAB:imagesci:imbmpinfo:fileOpen', filename, fopen_msg));
end
...
if (isempty(metadata.FormatSignature))
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:fileEmpty', filename));
end
...
switch FormatSignature
    case 'BM'
        ...
        if (isempty(headersize))
            fclose(fid);
            error(message('MATLAB:imagesci:imbmpinfo:getBmVersion'));
        end
        ...
        switch headersize
            ...
            otherwise
                ...
                if ((headersize > 12) && (headersize <= 64))
                    ...
                    fclose(fid);
                    error(message('MATLAB:imagesci:imbmpinfo:unhandledHeaderSize'));
                end
    ...
    case 'BA'
        ...
        fclose(fid);
        error(message('MATLAB:imagesci:imbmpinfo:getBaVersion'));

    otherwise

        fclose(fid);
        error(message('MATLAB:imagesci:imbmpinfo:unknownFormatSignature'));
...
switch bmpVersion
    ...
    otherwise
	fclose(fid);
        error(message('MATLAB:imagesci:imbmpinfo:readBMPInfo'));
...
if (isempty(metadata.Width) || isempty(metadata.Height))
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:readWin2xBitmapHeader'));
end
...
if (isempty(metadata.Width) || isempty(metadata.Height))
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:readWin3xBitmapHeader'));
end
...
switch compNum
    ...
    otherwise
        fclose(fid);
        error(message('MATLAB:imagesci:imbmpinfo:unrecognizedWin3xCompression'));
...
if (metadata.NumColormapEntries > 0)
    ...
    if (count ~= metadata.NumColormapEntries*3)
        fclose(fid);
        error(message('MATLAB:imagesci:imbmpinfo:readVersion2xColormap'));
    end
...
if (metadata.NumColormapEntries > 0)
    ...
    if (count ~= metadata.NumColormapEntries*4)
        fclose(fid);
        error(message('MATLAB:imagesci:imbmpinfo:readVersion3xColormap'));
    end
...
if ((isequal(metadata.CompressionType, 'Version 3 (Microsoft Windows NT)')) && ...
    ...
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:readWin3xInfo'));
...
if (((metadata.BitDepth == 16) || (metadata.BitDepth == 32)) && ...
    ...
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:readWin4xInfo'));
end
...
if (isempty(units))
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:truncatedUnits'));
end
...
if (isempty(halftoning))
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:truncatedHalftoning'));
end
...
if (isempty(encoding))
    fclose(fid);
    error(message('MATLAB:imagesci:imbmpinfo:truncatedEncoding'));
end
...
if (metadata.Width < 0)
    error(message('MATLAB:imagesci:imbmpinfo:badImageDimensions'));
end

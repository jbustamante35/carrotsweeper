...
%   $Revision: 1.1.6.7 $  $Date: 2011/05/17 02:27:43 $

[fid, msg] = fopen(filename, 'r', 'ieee-le');  % CUR files are little-endian
if (fid == -1)
    error(message('MATLAB:imagesci:imcurinfo:fileOpen', filename, msg));
end
...
if (isempty(sig))
    fclose(fid);
    error(message('MATLAB:imagesci:imcurinfo:fileEmpty', filename));
end
    
if (~isequal(sig, [0; 2]))
    fclose(fid);
    error(message('MATLAB:imagesci:imcurinfo:notCURFile', filename));
end
...
for p = 1:imcount
    ...
    if (count ~= info(p).NumColormapEntries*4)
        fclose(fid);
        error(message('MATLAB:imagesci:imcurinfo:truncatedColormap'));
    end
    ...
    if ((info(p).Width < 0) || (info(p).Height < 0))
        fclose(fid);
        error(message('MATLAB:imagesci:imcurinfo:badImageDimensions'));
    end
end

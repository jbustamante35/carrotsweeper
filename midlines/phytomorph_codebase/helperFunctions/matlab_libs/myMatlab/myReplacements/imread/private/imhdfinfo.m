...
%   $Revision: 1.1.6.8 $  $Date: 2011/05/17 02:27:46 $

% Find full filename
[fid, msg] = fopen(filename,'r','ieee-be');
if (fid == -1)
    error(message('MATLAB:imagesci:imhdfinfo:fileOpen', filename, msg));
end
...
fclose(fid);
if (isempty(val))
    error(message('MATLAB:imagesci:imhdfinfo:truncatedHeader'));
end
if (val ~= 235082497)
    error(message('MATLAB:imagesci:imhdfinfo:notHDFFile', filename));
end

...
%   $Revision: 1.1.6.9 $  $Date: 2011/05/17 02:27:58 $
...
if (~ischar(filename))
    error(message('MATLAB:imagesci:readcur:badFilename'));
end;
...
if (ischar(index))
    error(message('MATLAB:imagesci:readcur:badIndexType', index));
end

if (index < 1)
    error(message('MATLAB:imagesci:readcur:badIndex'))
end	

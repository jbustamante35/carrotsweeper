...
%  $Revision: 1.1.6.12 $  $Date: 2011/05/17 02:28:19 $
   ...
   switch (class(data))
     case {'logical', 'uint8', 'uint16', 'single', 'double'}
       % It's a valid datatype
     otherwise
       if (isnumeric(data) || islogical(data))
           error(message('MATLAB:imagesci:writepnm:invalidImageClass'));
       else
           error(message('MATLAB:imagesci:writepnm:notNumericInput', class( data )));
       end
   end
       
   nd = ndims(data);
   if nd > 3
      error(message('MATLAB:imagesci:writepnm:tooManyDims', nd));
   end
   ...
   if rem(length(varargin), 2)
      error(message('MATLAB:imagesci:writepnm:mismatchedParamValuePairs'));
   end

   for k = 1:2:length(varargin)
      param = lower(varargin{k});
      if ~ischar(param)
         error(message('MATLAB:imagesci:writepnm:badParameterName'));
      end
      idx = find(strncmp(param, paramStrings,numel(param)));
      if isempty(idx)
         error(message('MATLAB:imagesci:writepnm:unrecognizedParameter', param));
      elseif (length(idx) > 1)
         error(message('MATLAB:imagesci:writepnm:ambiguousParameter', param));
      end
      ...
      switch param
         ...
         case 'maxvalue'
            ...
            if any(size(maxval) ~= 1) || maxval ~= round(maxval) || maxval < 1
               error(message('MATLAB:imagesci:writepnm:badMaxValue'));
            end
         ...
         case 'encoding'
            ...
            elseif ~isempty(r) && (r == 1)
                ...
                error(message('MATLAB:imagesci:writepnm:badEncoding'));
            end
   ...
   switch format
      ...
      otherwise
         ...
         error(message('MATLAB:imagesci:writepnm:badImageFormatString'));
   end
...
function writepbm(data, map, filename, encoding)
   ...
   switch encoding
      ...
      otherwise
         % we should never get here, but just in case...
         error(message('MATLAB:imagesci:writepnm:invalidPbmEncoding'));
   end
...
function writepgm(data, map, filename, encoding, maxval)
   ...
   switch encoding
      ...
      otherwise
         % we should never get here, but just in case...
         error(message('MATLAB:imagesci:writepnm:invalidPgmEncoding'));
   end
...
function writeppm(data, map, filename, encoding, maxval)
   ...
   switch encoding
      ...
      otherwise
         % we should never get here, but just in case...
         error(message('MATLAB:imagesci:writepnm:invalidPpmEncoding'));
   end

function write_raw_data(filename, format, data, height, width, maxval)

   if numel(data)/size(data, 1) ~= 1
      error(message('MATLAB:imagesci:writepnm:rawDataMustBeColumnVector'));
   end
   ...
   if (fid < 0)
       error(message('MATLAB:imagesci:writepnm:fileOpen', filename));
   end
   ...
   switch format
      ...
      otherwise
         % we should never get here, but just in case...
         fclose(fid);
         error(message('MATLAB:imagesci:writepnm:invalidFormat'));
   end
   ...
   if ~strcmp(format, 'pbm')
      ...
      elseif maxval > 65535
         % we should never get here, but just in case...
         fclose(fid);
         error(message('MATLAB:imagesci:writepnm:asciiEncodingRequired'));
      end
   end
   ...
   if count < length(data)
       error(message('MATLAB:imagesci:writepnm:rawWriteFailed'));
   end

function write_plain_data(filename, format, data, height, width, maxval)

   if numel(data)/size(data, 1) ~= 1
      error(message('MATLAB:imagesci:writepnm:dataMustBeColumnVector'));
   end

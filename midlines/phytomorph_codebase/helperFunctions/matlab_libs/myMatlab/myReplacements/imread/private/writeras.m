...
%  $Revision: 1.1.6.12 $  $Date: 2011/05/17 02:28:20 $
   ...
   if rem(length(varargin), 2)
      error(message('MATLAB:imagesci:writeras:mismatchedParamValuePairs'));
   end

   for k = 1:2:length(varargin)
      param = lower(varargin{k});
      if ~ischar(param)
         error(message('MATLAB:imagesci:writeras:badParameterName'));
      end
      idx = find(strncmp(param, paramStrings, numel(param)));
      if isempty(idx)
         error(message('MATLAB:imagesci:writeras:unrecognizedParameter', param));
      elseif (length(idx) > 1)
         error(message('MATLAB:imagesci:writeras:ambiguousParameter', param));
      end
      ...
      switch param
         ...
         case 'type'
            type = varargin{k+1};
            if ~ischar(type)
               error(message('MATLAB:imagesci:writeras:badType'));
            end
            if ~ismember(type, {'standard', 'rgb', 'rle'})
               error(message('MATLAB:imagesci:writeras:invalidRasType', type));
            end
   ...
   if ~ismember(class(data), {'double', 'logical', 'single', 'uint8', 'uint16'})
      error(message('MATLAB:imagesci:writeras:invalidImageClass', class( data )));
   end
   if ~strcmp(class(map), {'double', 'single'})
      error(message('MATLAB:imagesci:writeras:invalidMapClass', class( map )));
   end
   if ~ismember(class(alpha), {'double', 'single', 'uint8', 'uint16'})
      error(message('MATLAB:imagesci:writeras:invalidAlphaClass', class( alpha )));
   end
   ...
   if islogical(data)
      ...
      if ~isempty(map)
         error(message('MATLAB:imagesci:writeras:bitmapWithColormap'));
      elseif ~isempty(alpha)
         error(message('MATLAB:imagesci:writeras:bitmapWithAlpha'));
      elseif ncomp ~= 1
...
%                           ' not "%s".'], class(data)));
   ...
   if fid < 0
      error(message('MATLAB:imagesci:writeras:fileOpen', filename));
   end

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function [profile] = nonSteadyStateFlowAnalysis(obj,para)
            ...
            profile.plottingValues.REGR_cMap = para.colorMap_strain;
            profile.plottingValues.VEL_cMap = para.colorMap_velocity;
        end
        % new analysis
        function [profile] = nSSnLFlowAnalysis(obj,para)
            % get points from position flow field
            sz = obj.position_field_midline.baseSize();
            pointList = obj.position_field_midline(:,:);
            pointList = reshape(pointList,[sz size(pointList,2)]);
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % new interpolation
            [domain,vP] = measureVelocityProfile(ipermute(pointList,[1 3 2]),1,1,5);
            svP = imfilter(vP,fspecial('average',[20 5]),'replicate');
            profile.velocityProfile = svP';
            profile.strainProfile = gradient(svP');
            profile.domain = domain;
            profile.type = 'nonSteadyState_nonLogistic'
        end
        % create flow profile
        function [profile] = create_flowProfile(obj,para)
            switch para.kinematicsType
                ...
                case 'nonSteadyState_nonLogistic'
                    profile = obj.nSSnLFlowAnalysis(para);
            end
        end
    ...
    methods (Static)
        ...
        function [fileList] = save_flowProfile(profile,outPort)
           switch profile.type
                ...
                case 'nonSteadyState_nonLogistic'
                    fileList = rootTipSequence.save_nSSnLKinematicsProfile(profile,outPort);
           end
        end
        % plot flow profile
        function [h] = plot_flowProfile(profile)
           switch profile.type
                ...
                case 'nonSteadyState_nonLogistic'
                    h = rootTipSequence.plot_nSSnLKinematicsProfile(profile);
           end
        end
        ...
        function [h] = plot_nonSteadyStateKinematicsProfile(profile)
            ...
            c1 = colorbar;
            colormap('jet');
            class(profile.plottingValues.VEL_cMap)
            caxis(profile.plottingValues.VEL_cMap);
            ...
            xlabel('Along Midline (px)');
            ylabel('Time (frames)');
            c2 = colorbar;colormap('jet');
            caxis(profile.plottingValues.REGR_cMap);
            ylabel(c2,'Strain (%/fr)');
            ax = axis;
            ax(4) = size(profile.spaceDomain,1);
            ax(3) = 1;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % plot steady state
        function [h] = plot_nSSnLKinematicsProfile(profile)
        
        end
        % save profile
        function [fileList] = save_nSSnLKinematicsProfile(profile,outPort)

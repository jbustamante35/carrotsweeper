classdef myG < viewable & sampleable
    ...
    methods
        ...
        function [obj] = myG(varargin)
            ...
            obj.N = myN();
            obj.E = phytoGeoSet('myEdge');
            
            
            [extraArgs, msg] = obj.parse_inputs(varargin);
            
            if isfield(extraArgs,'initN')
                bputNode(obj,extraArgs.initN);
            end
        end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function [] = sampleViewer(obj,domain,h,vProps)
            ...
            for e = 1:numel(obj.E)
                ...
                affine = rep(obj.E{e},[],'phytoAffine');
                % project affine into its "natural" setting
                affine.normalize(1);
                % for viewing of samples
                viewD = phytoGeo.affineX(domain.edge.view_rep,affine.d);
                % view curve
                curve = phytoCurve(viewD(1:end-1,:));
            ...
            for n = 1:numel(obj.N)
                ...
                affine = rep(nd.getElement(1),[],'phytoAffine');
                % project affine into its "natural" setting
                affine.normalize(1);
                % for viewing of samples
                viewD = phytoGeo.affineX(domain.node.view_rep,affine.d);
                curve = phytoCurve(viewD(1:end-1,:));
                curve.view(h,[],vProps.nodes);
            end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % sample image at domain
        function [s] = sample(obj,I,domain)
            % read image
            I = myReader(I);
            ...
            % sample the edges
            for e = 1:numel(obj.E)
                sE = obj.E{e}.sample(I,domain.edge);
                s.insertEdge(sE);
            end
            % sample the edges
            for e = 1:numel(obj.N)

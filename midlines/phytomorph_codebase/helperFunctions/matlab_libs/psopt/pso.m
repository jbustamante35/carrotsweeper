function [xOpt,fval,exitflag,output,population,scores] = ...
    pso(fitnessfcn,nvars,Aineq,bineq,Aeq,beq,LB,UB,nonlcon,options,initP)
warning off
...
elseif strncmpi(options.Display,'diag',4)
    options.Verbosity = 3 ;
elseif strncmpi(options.Display,'mydisp',6)
    %pop = state.Population(setdiff(1:n,find(state.OutOfBounds)),:);
end
...
elseif strncmpi(options.PopulationType,'myInit',2)
    state = psocreationuniform(options,nvars) ;
    state.Population = initP;
end
...
for k = 1:itr
    %tic
        if options.Verbosity == 2
            fprintf(['Updating generation ' num2str(k) ' of ' num2str(itr) ' of population size of ' num2str(n) ':' num2str(state.fGlobalBest(max(k-1,1))) '.\n'])
        ...
        if strcmpi(options.Vectorized,'off')
            ...
            parfor i = setdiff(1:n,find(state.OutOfBounds))
                tmp(i) = fitnessfcn(pop(i,:));
            end % for i
            state.Score = tmp';
...
%if options.Verbosity > 0, fprintf('\n\n%s\n',output.message) ; end
if options.Verbosity > 0, fprintf(output.message) ; end
% -------------------------------------------------------------------------

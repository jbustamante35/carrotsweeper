...
%%%
% notes: added 12/10/07 : ensemble of atoms/molecules is the object of
study. when in liquid form, i mean that the object is disolved in some
space. here the liquid serves as the space or container.  the key is the
space for the crystal form. the object is the components?  
%%%%%%
% 
...
%}
    ...
    elseif IN.op == 2
        ...
        IN = of2(IN);
    elseif IN.op == 3
        % from data and key - project to crystal
        % (key,data) -> (r_data)
        IN = of3(IN);
    end
end
...
function [IN] = operand_op1(IN)
    ...
    % NOTES (added.13.01.21): the large vector space mentioned above is the
    % space spanned by the basis elements from the tensor product of each
    % space.  
    SZ = size(IN.D);
    tempD = reshape(IN.D,[prod(SZ)/SZ(end) SZ(end)]);
...
% NOTES (added.??.??.??): as i look at this code: i see now that this function is the analog
...
function [IN] = operand_op2(IN)   
    ...
    for n = 1:ndims(IN.D)
        ...
        %if IN.key.op{2}.NC(n) > 0            
            ...
            [IN.key.op{2}.operAND.U{n} IN.key.op{2}.operAND.BV{n}] = pca(IN.D,IN.key.op{2}.NC(n));
        else
            %%%%%    
            % return "nothing" elements
            % return the iD elements of the algebra
            % largemem would be eye(n) vs smallmem 1,0
...
function [IN] = op1(IN,direc)        
    ...
    % unfolding/folding of tensor
    SZ = size(IN.D);
    IN.D = reshape(IN.D,[prod(SZ)/SZ(ndims(IN.D)) SZ(ndims(IN.D))]);

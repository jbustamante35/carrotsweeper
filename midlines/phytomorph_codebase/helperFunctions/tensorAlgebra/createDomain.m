function [para] = createDomain(para)
    ...
    switch para.type
        case 'disk'
            ...
            para.d = [];
            R = linspace(para.value{1}(1),para.value{1}(2),para.value{1}(3));
            for r = 1:numel(R)
                TH = para.value{2}(2) - para.value{2}(1);
                TH = linspace(para.value{2}(1),para.value{2}(2),round(TH*R(r)));
                cir = [R(r)*cos(TH(:)) R(r)*sin(TH(:)) ones(numel(TH),1)];
                para.d = [para.d;cir];
            end
            para.d = para.d';

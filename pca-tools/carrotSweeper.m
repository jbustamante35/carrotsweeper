...
%% carrotSweeper: run pipeline for PCA analysis, PC sweeper, and saving data
% This function runs the full pipeline for extracting BW images from the
% inputted directory (including subdirectories), and runs these images through
% PCA analysis of x-/y-coordinates with the desired number of Principal
% Components (PC).
%
% The algorithm then sweeps through the PCs by iteratively adding or
% subtracting each PC by a standard deviation, with the number of sweeps
% defined by the stps parameter. Lastly, the x/y PC scores are matched with
% their corresponding UID, which are extracted from the filename of each image.
%
% This is stored as a table and exported as a csv file. Need to make this
% more generalizable, so you could ask for any identifier (default is UID).
%
% Usage: [SweepData, TableData] = ...
%     carrotSweeper(dPath, 'numX', n, 'numY', m, 'SaveData', sv)
%
% Input:
...
%    numY: number of Principal Components for y-coordinates
%    Steps: standard deviation steps up and down (default is 5)
%    Vis: visualize outputs
%    SaveData: save figures in .fig and .tif, and save data in .mat file
%
% Output:
%    SweepData: output containing data from PCA and PC sweep
%    TableData: table format of requested ID and corresponding PC scores
%
% Example:
%    Outputs UID-Xscores-Yscores:
%    sb = '/home/jbustamante/LabData/scott_brainard';
%    dPath = sprintf('%s/scott_masks/180918/cal2018', sb);
%    [swpData, uidTable] = carrotSweeper(dPath);
%
%    Also outputs table in csv file
%    [swpData, uidTable] = carrotSweeper(dPath, 'SaveData', true);
%
%    Outputs Curvature-Xscores-Yscores and saves table in csv file
%        *See getToken function below to see which ID tags you can use
%    [swpData, crvTable] = ...
%        carrotSweeper(dPath, 'ID', 'Curvature', 'SaveData', true);
%
...
for fn = fieldnames(args)'
    ...
    % function to assign variables to this local workspace. See Alec Jacobson's
    % blog post at (http://www.alecjacobson.com/weblog/?p=3792)
    
    feval(@() assignin('caller', cell2mat(fn), args.(cell2mat(fn))));
end

%% Load path and create data store
I      = imageDatastore(dPath, 'IncludeSubfolders', 1, ...
...
% tbl_name  = sprintf('%s_%s-PCscores_%droots.csv', ...
%    tdate('s'), ID, length(TableData.UID));
tbl_name  = sprintf('%s_%s-PCscores_%droots.csv', ...
    tdate('s'), ID, length(TableData.(ID)));
...
% This could be more generalized by setting the keys cell string to all the
% names found in the filename with the {key_val} structure [do this later]
